 ...
 * Populate the database with comprehensive tutorial content and create search function
 */
export async function POST() {
  try {
    console.log('🚀 Setting up AI Search...');
    ...
    await supabaseAdmin.from('cursor_content').delete().neq('id', 'dummy');
    ...
    for (let i = 0; i < tutorialContent.length; i++) {
      ...
      try {
        ...
        console.error(`❌ Failed to process ${item.metadata.title}:`, itemError.message);
        results.push({ id: item.id, title: item.metadata.title, status: 'error', error: itemError.message });
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'AI Search setup completed!',
      summary: {
        totalItems: tutorialContent.length,
        successCount,
        errorCount,
        contentInDatabase: content?.length || 0
      },
      results: results
    });
